...
@router.callback_query(QUESTIONS_QUESTION__CD.filter(action="7"))
async def job_delivery_method(q: CallbackQuery, db_session: AsyncSession, state: FSMContext):
    ...
    await state.update_data(action=callback_data["action"],
                            proc_missrvenue=str(callback_data["question"]))
    ...
    await bot.send_message(
        q.from_user.id,
        text=mes.question_proc_turnover_text, reply_markup=quest_8_kb())
    await state.set_state(FilterCreateForm.proc_turnover)



@router.message(state=FilterCreateForm.proc_missrvenue)
async def job_delivery_method(message: Message, state: FSMContext, db_session: AsyncSession):
    if not  message.text.isdigit():
        return await message.answer(
            text="Введите целое число:",
            reply_markup=quest_7_kb())
    await state.update_data(
                        proc_missrvenue=message.text)        
    await bot.send_message(
