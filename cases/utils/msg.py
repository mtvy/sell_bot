...
def send_doc(log, bot: TeleBot, tid: str|int, txt: str, doc: str, mrkp=None, name=None) -> Message:
    try:
        log.debug(f"Doc:'{len(doc)}' Msg:'{txt}' Dest:{tid}")
        return bot.send_document(tid, doc, caption=txt, reply_markup=mrkp, visible_file_name=name)
    except Exception as err:
        log.error(f'{err}')
    return None
...
def get_ikb(log, btns: Dict[str, str], row_num=1) -> inlineKb:
    log.debug(f'Get InlineButtons:{btns}')
    key = inlineKb(row_width=row_num)
